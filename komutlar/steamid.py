...
allowed_channels = [1254064951174103155]
...
class SteamID(commands.Cog):
    ...
    async def steamid(self, ctx, steam_identifier: str):
        if ctx.channel.id not in allowed_channels:
            await ctx.send("Sadece katanın ayarladığı kanallarda çalışır.")
            return
        ...
        try:
            ...
            message = (
                ...
                f"Profili isteyen: {ctx.author.mention}\n"
                f"<a:latest:1254031840457785344> **Steam Profili:** {steam_profile_url}\n"
                f"<a:latest:1254031840457785344> **Profil Durumu:** `{profile_state}`\n"
                f"<a:latest:1254031840457785344> **Profil Oluşturulma Tarihi:** <t:{profile_created_unix}:R> | `{profile_created_human}`\n"
                f"<a:latest:1254031840457785344> **SteamID:** `{steam_steamid}`\n"
                f"<a:latest:1254031840457785344> **SteamID3:** `{steam_id31}`\n"
                f"<a:latest:1254031840457785344> **SteamID64:** `{steam_id64}`\n"
                f"<a:latest:1254031840457785344> **Kullanıcı Adı:** `{profile_name}`\n"
                f"<a:latest:1254031840457785344> **Konum:** `{profile_location}`\n"
                f"<a:latest:1254031840457785344> **Özel URL:** `{custom_url_username}`"
            )
